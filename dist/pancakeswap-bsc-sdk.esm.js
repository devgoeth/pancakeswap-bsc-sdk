...
import { getNetwork as getNetwork$1 } from '@ethersproject/networks';
...
var Pair = /*#__PURE__*/function () {
  ...
  Pair.getAddress = function getAddress(tokenA, tokenB, factoryAddress) {
    ...
    if (((_a = PAIR_ADDRESS_CACHE === null || PAIR_ADDRESS_CACHE === void 0 ? void 0 : PAIR_ADDRESS_CACHE[tokens[0].address]) === null || _a === void 0 ? void 0 : _a[tokens[1].address]) === undefined) {
      ...
      PAIR_ADDRESS_CACHE = _extends({}, PAIR_ADDRESS_CACHE, (_extends3 = {}, _extends3[tokens[0].address] = _extends({}, PAIR_ADDRESS_CACHE === null || PAIR_ADDRESS_CACHE === void 0 ? void 0 : PAIR_ADDRESS_CACHE[tokens[0].address], (_extends2 = {}, _extends2[tokens[1].address] = getCreate2Address(factoryAddress, keccak256(['bytes'], [pack(['address', 'address'], [tokens[0].address, tokens[1].address])]), INIT_CODE_HASH), _extends2)), _extends3));
    }

    return PAIR_ADDRESS_CACHE[tokens[0].address][tokens[1].address];
  };

  Pair.getAddressFromFactory = function getAddressFromFactory(tokenA, tokenB, exchangeAddress, provider) {
    try {
      if (provider === undefined) provider = getDefaultProvider(getNetwork(ChainId.MAINNET));
      var tokens = tokenA.sortsBefore(tokenB) ? [tokenA, tokenB] : [tokenB, tokenA]; // does safety checks

      return Promise.resolve(new Contract(exchangeAddress, factoryAbi, provider).getPair(tokens[0].address, tokens[1].address));
    } catch (e) {
...
var Fetcher = /*#__PURE__*/function () {
  ...
  Fetcher.fetchTokenData = function fetchTokenData(chainId, address, provider, symbol, name) {
    try {
      ...
      if (provider === undefined) provider = getDefaultProvider(getNetwork$1(chainId));
  ...
  Fetcher.fetchPairData = function fetchPairData(tokenA, tokenB, provider, factoryAddress) {
    if (factoryAddress === void 0) {
      factoryAddress = FACTORY_ADDRESS;
    }

    try {
      if (provider === undefined) provider = getDefaultProvider(getNetwork$1(tokenA.chainId));
      !(tokenA.chainId === tokenB.chainId) ? process.env.NODE_ENV !== "production" ? invariant(false, 'CHAIN_ID') : invariant(false) : void 0;
      return Promise.resolve(Pair.getAddressFromFactory(tokenA, tokenB, factoryAddress, provider)).then(function (address) {
...
//# sourceMappingURL=pancakeswap-bsc-sdk.esm.js.map
